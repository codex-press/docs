...
const animate = function animate(tween) {
  tween.startTime = now();
  tween.active = true;
...
// performance.now() polyfill for i0S 8
export var now
if ('performance' in window && 'now' in window.performance)
  now = () => performance.now();
else {
  let nowOffset = Date.now();
  let hasNavigationStart = (
...
export const elasticOut = curry((b, e, t) => {
  ...
  // not sure if just removing D here is (e-b)orre(e-b)t... (e-b)onfusing
